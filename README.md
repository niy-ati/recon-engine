...
Reconciliation isn't a hypothetical line item — a [real Razorpay hotel-payments customer](https://www.linkedin.com/posts/aeijaz-sodawala-a2202a64_hoteltech-hospitalitytechnology-payments-share-7500577134541713408-vsug/) reports automatic reconciliation cut booking cancellations from 18% to 5%, payment failures by 40%, and saved staff 15 hours a week — the exact kind of manual-matching time this system exists to eliminate.
...
- **That evidence is now a persisted, re-runnable harness, not just a comment.** [`src/arbiter_eval.py`](src/arbiter_eval.py) — modeled on Razorpay's own published evaluation philosophy ([razorpay.com/blog/the-winner-doesnt-take-it-all](https://razorpay.com/blog/the-winner-doesnt-take-it-all): reject contaminated public benchmarks, build a small domain-specific harness instead, store raw results so a re-score never needs another model call, re-evaluate whenever the model changes rather than trust one frozen finding forever). `python src/arbiter_eval.py` runs the positional-bias case, the context-judgment case, and a plain OCR-typo case fresh, and writes both the raw per-case JSON and a markdown report to `eval/`. Last real run against `qwen2.5:0.5b`: 50% accuracy on the two cases with a real answer, mean confidence 1.0 when correct vs 0.0 when wrong (not the well-calibrated "lower confidence when wrong" pattern that would make confidence trustworthy), and the ambiguous no-signal case still came back at ~100% confidence — the same positional-bias failure, reproduced live, not just asserted from an old test run.
...
- **The gate is layered, not a single confidence check** — each one real, tested, and named: (1) a ledger row only becomes a Pass 4 candidate if no earlier, cheaper pass already claimed it; (2) the shortlist shown to the model is capped at 3 lookalikes, never the whole batch; (3) a malformed model response is rejected outright; (4) a candidate the model names that wasn't in its own shortlist is rejected as untrusted, not accepted; (5) confidence must clear 90%; (6) the tier that produced it must be on an explicit, empty-by-design trust list; (7) the candidate's own settlement amount must agree with the ledger row's amount within a sane tolerance, even after every other check passes — narration similarity alone never proves the amount is right; (8) once a ledger row is claimed, it can never be handed to a second settlement row in the same run. [PayScope](https://github.com/Drix10/payscope), a separate buildathon submission (payment-failure recovery, not reconciliation), converges on the identical philosophy independently — a deterministic policy engine with explicit safety gates before any autonomous action, plus an immutable audit trail. Two more independent examples from the same buildathon cohort, different problems again: [RazorRecover AI](https://www.linkedin.com/posts/sanjeev-kumar-1803t_fintech-ai-generativeai-ugcPost-7499528485238132736-xmNA/) (revenue recovery) pairs a 9-rule deterministic policy engine with an LLM that only reasons, never decides alone, and shows its confidence and reasoning on every call; [VETO](https://www.linkedin.com/posts/jslxh_ai-agenticai-aiagents-ugcPost-7500041420758470656-OcsU/) (checkout upselling) is built entirely around *declining* to act — 56% of its own upsell offers withheld, on purpose, with a logged reason for each. Four separate teams, four separate problems, the same conclusion about what production fintech actually needs from a model: restraint that's provable, not assumed.
- **Razorpay's own stated AI direction is the opposite extreme, deliberately — and it reaches the same conclusion about trust.** [Vulcan](https://www.linkedin.com/posts/razorpay_razorpay-artificialintelligence-fintech-activity-7498631537492508672-kwEe), their proprietary foundation model (~4 billion transactions, ~3 trillion data points, built with NVIDIA and AWS), unifies routing/fraud/risk into one model at a scale no local model or small team could reproduce, and rightly so for that job. But by their own account it shipped through a **shadow-mode phase — silently scored against live traffic before it was ever allowed to act — and even in production it issues "recommendations, not autonomous decisions," under stated mathematical guardrails.** A model that size, trained on that much real data, still doesn't get to decide on its own. This system's own empty `AUTO_APPLY_TRUSTED_TIERS` is the same call, made at a scale a small local model has to earn from zero, not from 4 billion transactions of track record. Same industry, same year, same answer: bigger doesn't mean unsupervised.
...
This is the same direction Razorpay's own CEO has staked the company's dashboard strategy on: ["the best interface may not be a better dashboard — it may be no dashboard at all... less navigation, less learning where things are, more intent → outcome."](https://www.linkedin.com/posts/harshilmathur_the-best-dashboard-might-be-no-dashboard-ugcPost-7500452436390596608-JI_k/) — Harshil Mathur, on their own Agentic Dashboard's first 20 weeks (7.8x growth in weekly queries, 5.3x in new merchants/week). The real critique that post's own comments raised — that a dashboard still earns its keep for surfacing an anomaly nobody thought to ask about, and that a system asking for trust owes real transparency into what it's reading — is exactly why this project ships **both**, not one instead of the other: the full Overview/Queue/Records audit trail keeps working for browsing and surfacing the unexpected, and every chat/voice answer is grounded in that same persisted data, down to which pass resolved a row and why (see `_explanation_for()`) — never a summary that hides what it's actually built on.
...
- **A settlement by date, and when the next one lands** — "settlement on the 5th," "what settled on august 5th" returns the real UTR and gross/MDR/GST/net breakdown for that day, resolved against this batch's own real `settlement_date` values, never a guessed date (a bare day ambiguous across more than one month says so honestly instead of picking one). "When's my next settlement" is deliberately *not* a time-series prediction — same reasoning as [Forward Cash Forecast](#forward-cash-forecast) below — it reports the real most-recent settlement in the batch, any amount currently `ON_HOLD_BY_RAZORPAY`, and Razorpay's own stated T+2 cycle. This is the same UTR + amount-breakdown-by-date shape [Razorpay's own Agentic Dashboard demo](https://www.linkedin.com/posts/razorpay-agenticdashboard-ugcPost-7471097195371511809-xPNh/) answers — built here against this project's real persisted data, not a mocked response.
...
- Razorpay's own [Payment Recovery voice agent](https://www.linkedin.com/posts/razorpay_razorpay-activity-7485926025374294016-aG6r) (lending, TPV failures) validates the same pattern this Voice Agent already uses in a different domain: explain what happened in plain language and guide the next step out loud, at the moment it's needed, instead of a person chasing a dashboard.

**Voice output tries [Kokoro](https://huggingface.co/posts/Xenova/503648859052804) first, a real 82M-parameter neural voice that runs entirely client-side via WASM (MIT licensed, zero cost, nothing leaves the browser — the same guarantee everything else here makes).** This is a strict progressive enhancement, not a replacement: Kokoro is built for bundler environments, this project has none by design, so loading it through a CDN import is genuinely untested territory. Every failure mode — network, WASM, an unexpected API shape — is caught and falls straight back to the browser's own built-in speech, exactly as it worked before. Verified live: a real audio blob generates and plays correctly when Kokoro loads; the fallback path was code-traced end to end (an automated click can't fully exercise it — Chrome's own speech-synthesis policy requires a trusted user gesture, which a real click has and a scripted one doesn't) and is the exact, unmodified call that already worked.
