...
- **A held-out-style accuracy check across the full batch, not the resolved-rate headline alone.** [`src/reconciliation_eval.py`](src/reconciliation_eval.py) scores every one of reconcile()'s 519 labeled rows against a ground-truth outcome `generate_data.py` records at the moment it generates the row, before reconcile() ever runs (`data/ground_truth.csv`). Real result: **96.3% overall accuracy**, 8 of 10 categories at 1.00 precision and 1.00 recall. The shortfall traces to one root cause: the cross-UTR fallback in Pass 1 only resolves when exactly one bank row uniquely matches on amount+date, and this batch's gross amounts are drawn from just 6 fixed price points over a ~20-day window -- wide enough for the demo, narrow enough that 500+ rows produce a handful of genuine amount+date coincidences. Pass 1 correctly declines to guess rather than risk a wrong match, so 9 of 15 UTR_LEVEL_MISMATCH cases and 5 of 19 duplicate-settlement pairs land labeled UNEXPLAINED or an extra UTR_LEVEL_MISMATCH instead of their intended category -- a synthetic-data diversity limit, not a defect in the matching logic. `python src/reconciliation_eval.py` reproduces the full per-category breakdown.
...
**Checked against the rest of Razorpay's own agent lineup, not just the flagship Reconciliation Agent, and the same gap holds throughout.** Of Agent Studio's eight launched agents, Settlement Insights "sends a daily settlement summary via WhatsApp to track payouts without checking dashboards" -- a notification, not a matcher. RazorpayX Agentic Banking's launched Reporting Agent "automatically generates reports and flags anomalies" and Bookkeeping Agent "posts accurate accounting entries to your ERP based on predefined rules" -- summarizing and posting, neither one matching a settlement against a merchant's own ledger line by line. Its two upcoming agents, Tax Payments and Reimbursement, cover TDS and expense claims, not GST-on-MDR. ([Source](https://razorpay.com/blog/agent-studio-ai-agents-by-razorpay/), [Source](https://razorpay.com/agentic-business-banking/))
