...
- **The gate is layered, not a single confidence check** — each one real, tested, and named: (1) a ledger row only becomes a Pass 4 candidate if no earlier, cheaper pass already claimed it; (2) the shortlist shown to the model is capped at 3 lookalikes, never the whole batch; (3) a malformed model response is rejected outright; (4) a candidate the model names that wasn't in its own shortlist is rejected as untrusted, not accepted; (5) confidence must clear 90%; (6) the tier that produced it must be on an explicit, empty-by-design trust list; (7) the candidate's own settlement amount must agree with the ledger row's amount within a sane tolerance, even after every other check passes — narration similarity alone never proves the amount is right; (8) once a ledger row is claimed, it can never be handed to a second settlement row in the same run. [PayScope](https://github.com/Drix10/payscope), a separate buildathon submission (payment-failure recovery, not reconciliation), converges on the identical philosophy independently — a deterministic policy engine with explicit safety gates before any autonomous action, plus an immutable audit trail. Different problem, same conclusion about what production fintech actually needs.
- **Razorpay's own stated AI direction is the opposite extreme, deliberately** — a proprietary foundational model trained on [nearly 4 billion transactions with NVIDIA and AWS](https://www.linkedin.com/posts/razorpay_razorpay-artificialintelligence-fintech-activity-7500132279264915456-3C0Y), for payment routing, fraud prevention, and checkout personalization: broad, learned, judgment-heavy work at a scale no local model or small team could reproduce, and rightly so for that job. Reconciliation is a different job — matching two records against each other is a narrow, checkable decision, and this system's own gate exists because a small local model earns none of that broad trust automatically just for showing up. Same industry, same year, two honestly different answers to "how much should the model be trusted," because they're not the same question.
