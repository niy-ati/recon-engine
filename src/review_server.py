...
def render_about() -> str:
    ...
    <div class="overview" style="margin-bottom:var(--sp-8)">
      <div class="stats">
        ...
        {stat(ICON_LEDGER, "10", "named exception categories", "information", "#categories")}
    ...
    <div class="panel">
      ...
      <div class="panel-body">
        <div class="finding-list">
          ...
          {finding(ICON_ALERT, "The AFA mandate threshold is a real RBI rule, not an assumption", 'A subscription renewal above the RBI&#39;s <a href="https://www.rbi.org.in/scripts/NotificationUser.aspx?Id=13374" target="_blank" rel="noopener" style="color:var(--primary-strong);font-weight:600">₹15,000 e-mandate threshold</a> (Digital Payments -- E-Mandate Framework, 2026, RBI/DPSS/2026-27/396) needs a compliant step-up re-authentication, not a blind retry. That framework also carves out a ₹1,00,000 limit for insurance, mutual funds, and credit card bills specifically -- this batch&#39;s case is a plain subscription renewal, so it&#39;s correctly modeled against ₹15,000, not the higher figure. `AFA_MANDATE_HOLD` reads this off the ledger narration directly.', "tone-information")}
          {finding(ICON_LEDGER, "The third source resolves 12% of the batch on its own", 'Razorpay&#39;s own published Reconciliation Agent checks two sources: a bank statement against Razorpay&#39;s settlement records. <mark>65 of 525 rows (12.4%)</mark> in this batch are resolved or explained only because this system also reads the merchant&#39;s own ledger, including 3 rows with a perfectly clean UTR/amount/date match that a two-source tool would already call done.', "tone-positive")}
          ...
          {finding(ICON_GATEWAY, "Checked against the rest of Razorpay's own agent lineup too", 'Not just the flagship Reconciliation Agent. Settlement Insights <a href="https://razorpay.com/blog/agent-studio-ai-agents-by-razorpay/" target="_blank" rel="noopener" style="color:var(--primary-strong);font-weight:600">sends a daily WhatsApp summary</a>, not a matcher. The <a href="https://razorpay.com/agentic-business-banking/" target="_blank" rel="noopener" style="color:var(--primary-strong);font-weight:600">Reporting Agent</a> flags anomalies in reports, not row-by-row against a ledger. Cashflow Forecaster predicts 3-7 days forward from transaction history; this system&#39;s own Forward Cash Forecast answers a different question, how much unlocks from what&#39;s already verified, not a prediction. The Tax Payments Agent (TDS) is still listed as upcoming, not shipped. None of the eight launched or two upcoming agents check GST-on-MDR correctness or match three sources line by line.', "tone-positive")}
    ...
    <div class="panel" id="accuracy">
      <div class="panel-head"><h2 style="margin:0">Held-out accuracy, not just a resolved-rate headline</h2></div>
      <div class="panel-body">
        <div class="finding-list">
